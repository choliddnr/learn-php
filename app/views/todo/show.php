...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="bg-white p-8 rounded shadow-md max-w-md w-full">
        <h1 class="text-2xl font-bold mb-4 text-gray-800"><?= htmlspecialchars($task['title']) ?></h1>

        <p class="mb-2 text-gray-600"><strong>Description:</strong> <?= htmlspecialchars($task['description']) ?></p>

        <p class="mb-2 text-gray-600"><strong>Status:</strong>
            <span class="inline-block px-2 py-1 rounded 
                <?= $task['status'] === 'ongoing' ? 'bg-yellow-100 text-yellow-800' : ""; ?>
                <?= $task['status'] === 'delayed' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' ?>">
                <?= htmlspecialchars($task['status']) ?>
            </span>
        </p>

        <p class="mb-6 text-gray-600"><strong>Deadline:</strong> <?= getTimeLeft($task['deadline']) ?></p>
