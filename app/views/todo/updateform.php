...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
    <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        ...
        <form class="space-y-4" action="/todo/<?= $task['id'] ?>" method="POST">
            <!-- Title -->
            <div>
            ...
            <div>
                ...
                <input type="datetime-local" id="deadline" name="deadline"
                    value="<?= date('Y-m-d\TH:i', $task['deadline']) ?>"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            </div>
            ...
            <div>
                ...
                <select id="status" name="status" value="ongoing"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    <option <?= htmlspecialchars($task['status']) === "delayed" ? "selected" : "" ?> value="delayed">
                        Delayed
                    </option>
                    <option <?= htmlspecialchars($task['status']) === "ongoing" ? "selected" : "" ?> value="ongoing">
                        Ongoing
                    </option>
                    <option <?= htmlspecialchars($task['status']) === "done" ? "selected" : "" ?> value="done">Done
                    </option>
                </select>
            </div>
